...
- 图片输入优先于文字路径处理：支持单张图片文件路径和 Windows 原始位图；外部图片复制到 Obsidian 当前附件目录后必须使用 Vault 相对 `![[...]]` 嵌入，禁止把临时绝对路径写进代码块。
- 图片嵌入默认使用 Obsidian 宽度参数 `|300`，只限制显示宽度，不缩放或改写原始图片文件。
- 图片附件必须使用唯一文件名。闪念写入失败或新建笔记取消时，只清理本次新建且尚未被引用的附件；Vault 内原有图片不得删除或重复复制。
...
- AHK 设置界面同一横排混用 `CheckBox` 与 `Hotkey` 时，必须显式指定相同 `yp` 和控件高度；不得依赖不同控件的默认高度连续推算纵坐标。
